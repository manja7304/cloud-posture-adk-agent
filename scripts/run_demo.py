...
def main() -> int:
    ...
    banner("Cloud Posture ADK Agent")
    print(f"{DIM}Pattern: Google ADK Tool Calling · USE_MOCK_LLM=true (no Ollama/Docker required){RESET}\n")
    section("Agent API demo")
    from fastapi.testclient import TestClient
    from src.api.main import app

    client = TestClient(app)
    request_body = {'query': 'check posture', 'context': {'control_id': 'CIS-1.1'}}
